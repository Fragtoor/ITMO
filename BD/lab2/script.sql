...
WITH avg_age_1101 AS (
    SELECT MAX(EXTRACT(YEAR FROM AGE(NOW()::DATE, "Н_ЛЮДИ"."ДАТА_РОЖДЕНИЯ"::DATE))) as age
    FROM "Н_ЛЮДИ"
...
HAVING AVG(EXTRACT(YEAR FROM AGE(NOW()::DATE, "Н_ЛЮДИ"."ДАТА_РОЖДЕНИЯ"::DATE))) < (SELECT age FROM avg_age_1101);
