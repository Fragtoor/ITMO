...
SELECT "Н_ОЦЕНКИ"."ПРИМЕЧАНИЕ", "Н_ВЕДОМОСТИ"."ДАТА" FROM "Н_ВЕДОМОСТИ"
INNER JOIN "Н_ОЦЕНКИ" ON "Н_ОЦЕНКИ"."КОД" = "Н_ВЕДОМОСТИ"."ОЦЕНКА"
WHERE
"Н_ОЦЕНКИ"."ПРИМЕЧАНИЕ" > 'неявка' AND
"Н_ВЕДОМОСТИ"."ДАТА"::date = '1998-01-05' AND
...
WHERE 
    ...
    "Н_ОБУЧЕНИЯ"."ЧЛВК_ИД"::integer < 105590;
...
WHERE "Н_ЛЮДИ"."ИНН" IS NULL AND
"Н_УЧЕНИКИ"."ГРУППА"::integer = 3102;
...
HAVING AVG(EXTRACT(YEAR FROM AGE(NOW()::DATE, "Н_ЛЮДИ"."ДАТА_РОЖДЕНИЯ"::DATE))) < (SELECT EXTRACT(YEAR FROM AGE(NOW()::DATE, MIN("Н_ЛЮДИ"."ДАТА_РОЖДЕНИЯ")::DATE)) FROM "Н_ЛЮДИ"
LEFT JOIN "Н_ОБУЧЕНИЯ" ON "Н_ОБУЧЕНИЯ"."ЧЛВК_ИД" = "Н_ЛЮДИ"."ИД"
LEFT JOIN "Н_УЧЕНИКИ" ON "Н_УЧЕНИКИ"."ЧЛВК_ИД" = "Н_ОБУЧЕНИЯ"."ЧЛВК_ИД"
WHERE "Н_УЧЕНИКИ"."ГРУППА"::integer = 1101);
...
INNER JOIN "Н_ОБУЧЕНИЯ" ON "СТУДЕНТЫ"."ЧЛВК_ИД" = "Н_ОБУЧЕНИЯ"."ЧЛВК_ИД"
...
WHERE "Н_НАПР_СПЕЦ"."НАИМЕНОВАНИЕ" = 'Программная инженерия' AND "Н_ФОРМЫ_ОБУЧЕНИЯ"."ИМЯ_В_ИМИН_ПАДЕЖЕ" IN ('очная', 'заочная') AND EXISTS (SELECT 'GOYDA'
